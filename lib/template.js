...
module.exports = class Template {
	...
	static async download(pxls, templateURL) {
		if(templateURL.indexOf("#") === -1) throw new Error("need template data");
		const params = new Map(templateURL.substring(templateURL.indexOf("#") + 1).split("&").map(e => e.split("=")).map(e => [e[0], decodeURIComponent(e[1])]));
